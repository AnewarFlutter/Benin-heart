...
function updateFeaturesDi(module, feature) {
    const diFilePath = path.join(process.cwd(), 'src', 'di', 'features_di.ts');

    const importStatements = `
import { ${capitalize(feature)}Controller } from "@/adapters/${module}/${feature}/${feature}_controller";
import { RestApi${capitalize(feature)}DataSourceImpl } from "@/modules/${module}/${feature}/data/datasources/rest_api_${feature}_data_source_impl";
import { ${capitalize(feature)}RepositoryImpl } from "@/modules/${module}/${feature}/data/repositories/${feature}_repository_impl";
import { Get${capitalize(feature)}ByIdUseCase } from "@/modules/${module}/${feature}/domain/usecases/get_${feature}_by_id_usecase";
`;
...
const createFileContent = (module, feature, type) => {
    switch (type) {
        case 'actions':
            return `"use server";

import { featuresDi } from "@/di/features_di";
import { Entity${capitalize(feature)} } from "@/modules/${module}/${feature}/domain/entities/entity_${feature}";
...
`;
        case 'module_readme':
            return `# ${capitalize(module)} Module
...
async function createFeatureStructure(module, feature) {
    const rootPath = path.join(process.cwd(), 'src');

    const modulePath = path.join(rootPath, 'modules', module);
    const featurePath = path.join(rootPath, 'modules', module, feature);
    const actionsPath = path.join(rootPath, 'actions', module, feature);
    const adaptersPath = path.join(rootPath, 'adapters', module, feature);
    ...
    const exampleFiles = [
        { path: path.join(featurePath, 'data', 'datasources', `${feature}_data_source.ts`), content: createFileContent(module, feature, 'data_source') },
        { path: path.join(featurePath, 'data', 'datasources', `rest_api_${feature}_data_source_impl.ts`), content: createFileContent(module, feature, 'rest_api_data_source_impl') },
        { path: path.join(featurePath, 'data', 'datasources', `firebase_${feature}_data_source_impl.ts`), content: createFileContent(module, feature, 'firebase_data_source_impl') },
        { path: path.join(featurePath, 'data', 'datasources', `mongodb_${feature}_data_source_impl.ts`), content: createFileContent(module, feature, 'mongodb_data_source_impl') },
        { path: path.join(featurePath, 'data', 'datasources', `supabase_${feature}_data_source_impl.ts`), content: createFileContent(module, feature, 'supabase_data_source_impl') },
        { path: path.join(featurePath, 'data', 'models', `model_${feature}.ts`), content: createFileContent(module, feature, 'model') },
        { path: path.join(featurePath, 'data', 'repositories', `${feature}_repository_impl.ts`), content: createFileContent(module, feature, 'repository_impl') },

        { path: path.join(featurePath, 'domain', 'entities', `entity_${feature}.ts`), content: createFileContent(module, feature, 'entity') },
        { path: path.join(featurePath, 'domain', 'repositories', `${feature}_repository.ts`), content: createFileContent(module, feature, 'repository') },
        { path: path.join(featurePath, 'domain', 'usecases', `get_${feature}_by_id_usecase.ts`), content: createFileContent(module, feature, 'usecase') },
        { path: path.join(featurePath, 'domain', 'enums', `${feature}_enums.ts`), content: createFileContent(module, feature, 'enums') },

        { path: path.join(featurePath, 'types.ts'), content: createFileContent(module, feature, 'types') },
        { path: path.join(featurePath, `${feature.toUpperCase()}_README.md`), content: createFileContent(module, feature, 'feature_readme') },

        { path: path.join(modulePath, `${module.toUpperCase()}_README.md`), content: createFileContent(module, feature, 'module_readme') },

        { path: path.join(actionsPath, 'actions.ts'), content: createFileContent(module, feature, 'actions') },
        { path: path.join(adaptersPath, `${feature}_controller.ts`), content: createFileContent(module, feature, 'controller') },
    ];
...
// Récupère le module et toutes les features passées en arguments
const [moduleName, ...featureNames] = process.argv.slice(2);

if (!moduleName || featureNames.length === 0) {
    console.error('Usage: node feature_module_creator_script.js <moduleName> <featureName1> [<featureName2> ...]');
    process.exit(1);
}

// Boucle sur chaque feature et crée sa structure
for (const featureName of featureNames) {
    createFeatureStructure(moduleName, featureName).catch(console.error);
    updateFeaturesDi(moduleName, featureName);
}
