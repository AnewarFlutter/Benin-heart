...
function removeFeature(module, feature) {
    const pathsToRemove = [
        path.join(process.cwd(), 'src', 'modules', module, feature),
        path.join(process.cwd(), 'src', 'actions', module, feature),
        path.join(process.cwd(), 'src', 'adapters', module, feature),
    ];
...
const [moduleName, ...features] = process.argv.slice(2);

if (!moduleName || features.length === 0) {
    console.error('Usage: node feature_module_remover_script.js <moduleName> <featureName1> <featureName2> ...');
    process.exit(1);
}

// Supprimer chaque feature passée
features.forEach(feature => {
    removeFeature(moduleName, feature);
