...
const [moduleName, featureName, usecaseName, paramsStr = '', returnType = 'void', asyncFlag = ''] = args;
...
if (!moduleName || !featureName || !usecaseName) {
    console.error(
        'Usage: node module_feature_usecase_creator_script.js <moduleName> <featureName> <usecaseName> [params] [returnType] [async] [--related feature]'
    );
    process.exit(1);
}
...
const ensureImports = (filePath, imports, kind = 'model') => {
    ...
    imports.forEach(({ feature, model, entity }) => {
        ...
        if (feature === featureName) {
            // Même feature → chemin relatif
            if (kind === 'model') {
                // Si on est dans data (repositories ou datasources)
                if (filePath.includes(path.join('data', 'repositories')) || filePath.includes(path.join('data', 'datasources'))) {
                    importLine = `import { ${model} } from "../models/model_${feature}";`;
                } else {
                    // Sinon, on est dans domain
                    importLine = `import { ${model} } from "../models/model_${feature}";`;
                }
            } else {
                // ENTITY
                if (filePath.includes(path.join('data', 'repositories')) || filePath.includes(path.join('data', 'datasources'))) {
                    importLine = `import { ${entity} } from "../../domain/entities/entity_${feature}";`;
                } else {
                    importLine = `import { ${entity} } from "../entities/entity_${feature}";`;
                }
            }
        } else {
            // Autre feature → import absolu
            if (kind === 'model') {
                importLine = `import { ${model} } from "@/modules/${moduleName}/${feature}/data/models/model_${feature}";`;
            } else {
                importLine = `import { ${entity} } from "@/modules/${moduleName}/${feature}/domain/entities/entity_${feature}";`;
            }
        }
...
// Paths
const featurePath = path.join(process.cwd(), 'src', 'modules', moduleName, featureName);
...
// 4️⃣ Datasource impls
// Liste des implémentations possibles
const dataSourceImplTypes = ['rest_api', 'mongodb', 'firebase', 'supabase'];

dataSourceImplTypes.forEach((type) => {
    const dsImplPath = path.join(
        featurePath,
        'data',
        'datasources',
        `${type}_${featureName}_data_source_impl.ts`
    );

    if (fs.existsSync(dsImplPath)) {
        let dsImplContent = fs.readFileSync(dsImplPath, 'utf8');
        const mappedReturnType = mapEntityToModel(returnType, featureName, relatedFeature);
        const mappedParams = paramList
            .map((p) => `${p.name}: ${mapEntityToModel(p.type, featureName, relatedFeature)}`)
            .join(', ');
...
`;
        ...
        if (!dsImplContent.includes(`${methodName}(`)) {
            ...
            fs.writeFileSync(dsImplPath, dsImplContent);
            console.log(`Updated datasource implementation: ${dsImplPath}`);

            const imports = collectModelImports(paramList, returnType, featureName, relatedFeature);
            ensureImports(dsImplPath, imports, 'model');
        }
    }
});
...
// 6️⃣ Controller
const controllerPath = path.join(process.cwd(), 'src', 'adapters', moduleName, featureName, `${featureName}_controller.ts`);
...
if (fs.existsSync(controllerPath)) {
    ...
    entityImportsInController.forEach(({ feature, entity }) => {
        const importLine = `import { ${entity} } from "@/modules/${moduleName}/${feature}/domain/entities/entity_${feature}";`;
    ...
    const usecaseImportLine = `import { ${capitalize(usecaseName)}UseCase } from "@/modules/${moduleName}/${featureName}/domain/usecases/${toSnakeCase(usecaseName)}_usecase";`;
    ...
    if (constructorMatch) {
        ...
        // 🔍 Vérifier si le usecase existe déjà dans le constructeur
        const alreadyInjected = constructorContent.includes(`${toCamelCase(usecaseName)}UseCase`);

        if (!alreadyInjected) {
            if (constructorContent.endsWith(',')) {
                constructorContent = constructorContent.slice(0, -1);
            }
...
if (fs.existsSync(featuresDiPath)) {
    ...
    if (controllerMatch) {
        ...
        // 🔍 Vérifier si le usecase est déjà injecté
        const alreadyInjected = constructorContent.includes(usecaseVar);
        ...
        if (!alreadyInjected) {
            ...
            diContent = diContent.replace(
                controllerRegex,
                `const ${controllerVar} = new ${capitalize(featureName)}Controller(\n    ${newConstructorContent}\n)`
            );
        }
    }
