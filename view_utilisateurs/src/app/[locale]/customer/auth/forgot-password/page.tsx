...
import { forgotPasswordConfig } from "./_components/forgot-password.config"
...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (data: { identifier: string; loginType: string }) => {
    const success = await forgotPassword(data.identifier)
    if (success) {
      router.push(`${APP_ROUTES.auth.otp}?mode=forgot`)
    }
  }
      ...
      <div className="flex flex-col gap-4 p-6 md:p-10">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="w-full max-w-md">
            <ForgotPasswordForm config={forgotPasswordConfig} onSubmit={handleSubmit} />
          </div>
        </div>
      </div>
      <div className="bg-muted relative hidden lg:block">
        <img
          src={APP_IMAGES.auth.forgotPasswordBackground}
