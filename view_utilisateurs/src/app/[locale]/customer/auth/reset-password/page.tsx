...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (data: { password: string; confirmPassword: string }) => {
    if (!pendingEmail) {
      toast.error("Session expirée. Recommencez la réinitialisation.")
      router.push(APP_ROUTES.auth.forgotPassword)
      return
    }
    const success = await resetPassword(pendingEmail, '', data.password, data.confirmPassword)
    if (success) {
      ...
      <div className="flex flex-col gap-4 p-6 md:p-10">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="w-full max-w-md">
            <ResetPasswordForm config={resetPasswordConfig} onSubmit={handleSubmit} />
          </div>
        </div>
      </div>
